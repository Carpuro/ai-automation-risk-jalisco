...
| `worker_exposure_profile` | 19 | grupo | Exposición DBOE/DEOE ponderada por perfil del trabajador (educación, quintil, formalidad, sexo, edad, zona; ENOE fac_tri). Gradiente espejo por educación (DBOE −1.00→+0.50, DEOE +0.47→−0.54); riesgo con género (mujeres→LLM, hombres→robots). `analysis/worker_profile.py` |
| `h4_adoption_test` | 11 | coeficiente | **Test H4 (adopción realizada)**: panel CE 19 sectores × 4 transiciones censales 2003-2023, FE de periodo, SEs cluster-sector. IRA rezagado → profundización de capital (β=+0.143, p=.019) y caída de labor share (β=−0.040, p=.001) SIGNIFICATIVOS; cómputo/empleo con signo correcto pero n.s. (N=76). Cuadrantes: expuesto+rentable +0.6%/a empleo vs +3.7%/a no-expuesto+no-rentable. `analysis/h4_adoption_test.py` |
| `perception_trend` | 4 | oleada | % espera desplazamiento robot/IA (ponderado), armonizado entre oleadas. ⚠️ 2018 P61N es escala TIMELINE (no/1a/5a/10a+), NO agree — expects={2,3,4}, umbral laxo; 2017 es marco societal (serie aparte). `analysis/perception_latinobarometro.py` |
| `perception_breakdown_2023` | 8 | grupo | Desglose 2023 por sexo/edad/internet. Mismo script |
| `municipal_exposure` | 125 | municipio | DBOE/DEOE/presión-2030 ponderados por empleo CE2023 (mezcla sectorial formal × exposición sectorial). Mapa en `figures/municipal_exposure_map.png`. ⚠️ Artefacto de municipios chicos: filtrar por `ce_workers` (≥20k: metro = polo cognitivo, corredor industrial = polo físico). `build_municipal_exposure.py` |
